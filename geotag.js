...
server.on('connection', function(socket) {
 socket.on('message', function(data) {
  ...
  //redisにデータ格納
  config.redisClient.sadd(data.sub,data.time + ":" + ('00' + d.getMilliseconds()).slice( -3 ) + " | " + JSON.stringify(geo))

  //mysqlにデータ格納
  var updateSql = "UPDATE member_list set location = ?, geo_flag = 1 where sub = ?;"
  var updateQuery = config.connection.query(updateSql,[JSON.stringify(geo),data.sub]);
  updateQuery
  ...
  getQuery
   ...
   console.log("卍卍卍卍LOCATION RULE PUSH卍卍卍卍")
   rule.exacLocationRule(rows.push_item_id,data.sub);
  });
 });
});
